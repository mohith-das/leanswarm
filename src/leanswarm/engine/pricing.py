...
def estimate_run(
    ...
    - Calls: 1 world_extraction (cheap tier, LIVE ONLY — single pass over up to
      EXTRACTION_MAX_CHARS of the seed, no chunking) + 1 persona_batch (cheap tier,
      agent-count-scaled) + 1 bootstrap (standard tier)
      + rounds * batches_per_tick agent-batch (cheap tier) + up to rounds
      memory-summary calls (standard tier) + 1 synthesis (flagship tier). Chat and
      full-report calls are on-demand, not in this estimate.
    - Early convergence can reduce tick count. min_ticks = min(2, rounds), max_ticks = rounds.
    - Tokens:
      extraction: 350 + min(seed_chars, EXTRACTION_MAX_CHARS)//4 in / 450 out
      persona batch: 250 + 45 * max_agents in / 40 * max_agents + 60 out
    ...
    prompt_persona = 250 + 45 * max_agents
    comp_persona = 40 * max_agents + 60

    calls_min = 1 + 1 + 1 + (min_ticks * batches_per_tick) + 1
    calls_max = 1 + 1 + 1 + (max_ticks * batches_per_tick) + max_ticks + 1
    ...
    if unpriced_models:
        ...
        # cheap_model: extraction + persona + agent batch
        c_extraction = cost_usd(cheap_model, prompt_extraction, comp_extraction) or 0.0
        c_persona = cost_usd(cheap_model, prompt_persona, comp_persona) or 0.0
        ...
        cost_min_usd = c_extraction + c_persona + c_boot + (min_ticks * batches_per_tick * c_batch) + c_synth_min
        cost_max_usd = (
            c_extraction + c_persona + c_boot

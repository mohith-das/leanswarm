...
_RESPONSE_MODELS: dict[TaskType, type[BaseModel]] = {
    ...
    TaskType.PERSONA_BATCH: PersonaBatchResponse,
    ...
    TaskType.AGENT_CHAT: ChatResponse,
    TaskType.REPORT_CHAT: ChatResponse,
    TaskType.FULL_REPORT: FullReportResponse,
}
...
class LiteLLMRouter:
    ...
    def _tier_for_task(self, task_type: TaskType) -> ModelTier:
        if task_type in (TaskType.PREDICTION_SYNTHESIS, TaskType.FULL_REPORT):
            return ModelTier.FLAGSHIP
        if task_type in (TaskType.AGENT_BATCH, TaskType.WORLD_EXTRACTION, TaskType.PERSONA_BATCH, TaskType.AGENT_CHAT):
    ...
    def _mock_response(self, task_type: TaskType, payload: Mapping[str, Any]) -> dict[str, Any]:
        ...
        if task_type is TaskType.PERSONA_BATCH:
            return {"personas": []}

        if task_type is TaskType.AGENT_CHAT:
            name = str(payload.get("agent_name", "The agent"))
            stance = str(payload.get("stance", "") or "watching how this unfolds")
            message = str(payload.get("message", ""))
            terms = self._extract_terms(message + " " + str(payload.get("question", "")), limit=3)
            focus = self._format_terms(terms)
            signature = self._signature(name, message, "agent_chat")
            openers = ["Honestly,", "From where I stand,", "Look —", "The way I see it,"]
            return {
                "reply": (
                    f"{openers[signature % len(openers)]} my read on {focus} hasn't changed much: "
                    f"{stance.rstrip('.')}. Ask me after the next round of news."
                )
            }

        if task_type is TaskType.REPORT_CHAT:
            message = str(payload.get("message", ""))
            prediction = str(payload.get("prediction", "the forecast"))
            tick_count = int(payload.get("tick_count", 0) or 0)
            terms = self._extract_terms(message, limit=3)
            return {
                "reply": (
                    f"Based on the {tick_count} simulated tick(s), the report's core finding stands: "
                    f"{prediction[:160]} Regarding {self._format_terms(terms)}: the simulation offers "
                    f"limited additional signal beyond what is in the rationale."
                )
            }

        if task_type is TaskType.FULL_REPORT:
            prediction = str(payload.get("prediction", "the forecast"))
                ...
                "sections": [
                    {
                        ...
                        "content": (
                            f"This report analyzes the simulation's question: '{question}'. "
                            f"After {tick_count} tick(s), {prediction.lower().rstrip('.')}."
                        ),
                    },
                    {
                        "heading": "Simulation Dynamics",
                        "content": (
                            f"Over {tick_count} tick(s), the simulation converged with "
                            f"an overall confidence of {round(confidence * 100)}%. "
                            f"The rationale cites {len(rationale)} key factors."
                        ),
                    },
                    {
                        "heading": "Risks & Uncertainty",
                        "content": (
                            "This forecast is generated by a deterministic mock engine and "
                            "reflects n-gram keyword scoring rather than live model reasoning. "
                            "Live mode with actual LLMs would produce a more nuanced report."
                        ),
                    },
                ],
            }
